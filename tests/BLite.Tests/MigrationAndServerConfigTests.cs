...
///   - BLiteMigration.ToMultiFileAsync() — single-file to multi-file migration
///   - BLiteMigration.ToSingleFileAsync() — multi-file to single-file migration
/// </summary>
public class MigrationAndServerConfigTests : IDisposable
{
    ...
    [Fact]
    public async Task ServerConfig_SmallBaseConfig_DatabaseFunctional()
    {
        ...
        await col.CreateIndexAsync("label", "idx_label");
        await engine.CommitAsync();
        Assert.Equal(20, await col.CountAsync());
        var found = await col.QueryIndexAsync("idx_label", "item5", "item5").ToListAsync();
        Assert.Single(found);
    }

    [Fact]
    public async Task ServerConfig_LargeBaseConfig_DatabaseFunctional()
    {
        ...
        Assert.Equal(50, await col.CountAsync());
        var d = await col.FindByIdAsync((BsonId)25);
        Assert.NotNull(d);
    ...
    // ─────────────────────────────────────────────────────────────────
    // BLiteMigration.ToMultiFileAsync
    // ─────────────────────────────────────────────────────────────────

    [Fact]
    public async Task ToMultiFile_SimpleCollections_DataMigratedCorrectly()
    {
    ...
    [Fact]
    public async Task ToMultiFile_WithSecondaryIndex_IndexRebuiltInIdxFile()
    {
        ...
        {
            var col = engine.GetOrCreateCollection("products", BsonIdType.Int32);
            var results = await col.QueryIndexAsync("idx_price", 25, 50).ToListAsync();
    ...
    [Fact]
    public async Task ToMultiFile_MultipleCollections_AllDataMigrated()
    {
        ...
        using (var engine = new BLiteEngine(dbPath))
        {
            ...
            for (int i = 1; i <= 20; i++)
            {
                await a.InsertAsync(a.CreateDocument(["_id", "v"], b2 => b2.AddId((BsonId)i).AddInt32("v", i)));
                await b.InsertAsync(b.CreateDocument(["_id", "v"], b2 => b2.AddId((BsonId)i).AddInt32("v", i * 2)));
            }
            await engine.CommitAsync();
        }
        ...
        using (var engine = new BLiteEngine(dbPath, targetConfig))
        {
            var a = engine.GetOrCreateCollection("alpha", BsonIdType.Int32);
            var b = engine.GetOrCreateCollection("beta",  BsonIdType.Int32);
            Assert.Equal(20, await a.CountAsync());
            Assert.Equal(20, await b.CountAsync());

            var da = await a.FindByIdAsync((BsonId)10);
            Assert.True(da!.TryGetInt32("v", out var va));
            Assert.Equal(10, va);

            var db2 = await b.FindByIdAsync((BsonId)10);
    ...
    [Fact]
    public async Task ToMultiFile_WithKvStore_KvEntriesMigrated()
    {
    ...
    [Fact]
    public async Task ToMultiFile_SourceFileNotFound_Throws()
    {
        ...
        await Assert.ThrowsAsync<FileNotFoundException>(async () =>
            await BLiteMigration.ToMultiFileAsync(Path.Combine(_tempDir, "notexist.db"), config));
    }

    [Fact]
    public async Task ToMultiFile_NoMultiFilePaths_Throws()
    {
        var dbPath = Path.Combine(_tempDir, "src_nop.db");
        File.WriteAllBytes(dbPath, Array.Empty<byte>());

        await Assert.ThrowsAsync<InvalidOperationException>(async () =>
            await BLiteMigration.ToMultiFileAsync(dbPath, PageFileConfig.Default));
    }

    // ─────────────────────────────────────────────────────────────────
    // BLiteMigration.ToSingleFileAsync
    // ─────────────────────────────────────────────────────────────────

    [Fact]
    public async Task ToSingleFile_SimpleCollection_DataMigratedCorrectly()
    {
        ...
        await BLiteMigration.ToSingleFileAsync(dbPath, multiConfig, singlePath);

        using (var engine = new BLiteEngine(singlePath))
        {
            var col = engine.GetOrCreateCollection("orders", BsonIdType.Int32);
            Assert.Equal(40, await col.CountAsync());
            var d = await col.FindByIdAsync((BsonId)20);
    ...
    [Fact]
    public async Task ToSingleFile_InPlace_ReplacesSourceFile()
    {
        ...
        {
            var col = engine.GetOrCreateCollection("items", BsonIdType.Int32);
            Assert.Equal(25, await col.CountAsync());
        }
    }

    [Fact]
    public async Task ToSingleFile_WithSecondaryIndex_IndexRebuiltInSingleFile()
    {
        ...
        using (var engine = new BLiteEngine(dbPath, multiConfig))
        {
            ...
            await engine.CommitAsync();
            await col.CreateIndexAsync("priority", "idx_priority");
            await engine.CommitAsync();
        }
        ...
        using (var engine = new BLiteEngine(singlePath))
        {
            ...
            var zeros = await col.QueryIndexAsync("idx_priority", 0, 0).ToListAsync();
            Assert.Equal(6, zeros.Count);
        }
    }

    [Fact]
    public async Task RoundTrip_SingleToMultiToSingle_DataPreserved()
    {
        ...
        using (var engine = new BLiteEngine(dbPath))
        {
            ...
            await engine.CommitAsync();
            await col.CreateIndexAsync("tag", "idx_tag");
            await engine.CommitAsync();
        }
        ...
        await BLiteMigration.ToSingleFileAsync(dbPath, multiConfig, singlePath2);
        ...
        using (var engine = new BLiteEngine(singlePath2))
        {
            ...
            var evens = await col.QueryIndexAsync("idx_tag", "even", "even").ToListAsync();
            var odds  = await col.QueryIndexAsync("idx_tag", "odd",  "odd").ToListAsync();
    ...
    [Fact]
    public async Task ToMultiFile_UsingServerFactory_WithSmallBaseConfig()
    {
        ...
        using (var engine = new BLiteEngine(dbPath, PageFileConfig.Small))
        {
            ...
            for (int i = 1; i <= 10; i++)
            {
                await col.InsertAsync(col.CreateDocument(["_id", "v"], b => b
