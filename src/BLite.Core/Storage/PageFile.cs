...
public sealed class PageFile : IPageStorage
{
    ...
    /// <summary>
    /// Creates a backup of this page file where every page is transparently transcrypted:
    /// decrypted from the source and re-encrypted with <paramref name="targetCrypto"/>.
    /// This allows a backup to use a different encryption key (or to add/remove encryption)
    /// while producing a fully readable <c>.db</c> file at <paramref name="destinationPath"/>.
    /// <para>
    /// The method must only be called while the engine-level commit lock is held so that
    /// no concurrent write can change the file size or page contents during the copy.
    /// </para>
    /// </summary>
    /// <param name="destinationPath">Full path to the target file.</param>
    /// <param name="targetCrypto">
    /// Encryption provider for the destination file.
    /// <see cref="ICryptoProvider.GetFileHeader"/> is called once to generate fresh key
    /// material (e.g. a new random salt) for the backup.  Pass
    /// <see cref="NullCryptoProvider.Instance"/> to produce a plaintext copy.
    /// </param>
    /// <param name="ct">Cancellation token.</param>
    internal async Task TranscryptBackupAsync(string destinationPath, ICryptoProvider targetCrypto, CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(destinationPath))
            throw new ArgumentException("Destination path cannot be null or whitespace.", nameof(destinationPath));
        if (targetCrypto == null)
            throw new ArgumentNullException(nameof(targetCrypto));

        if (!await _asyncLock.WaitAsync(WriteLockTimeoutMs, ct).ConfigureAwait(false))
            throw new TimeoutException("Timed out acquiring PageFile async lock (TranscryptBackupAsync).");
        try
        {
            if (_fileStream == null)
                throw new InvalidOperationException("PageFile is not open.");

            // 1. Flush dirty pages to disk (same as BackupAsync).
            _fileStream.Flush(flushToDisk: true);

            // 2. Prepare destination directory and file.
            var destDir = Path.GetDirectoryName(destinationPath);
            if (!string.IsNullOrEmpty(destDir))
                Directory.CreateDirectory(destDir);

            int targetPhysPageSize = _config.PageSize + targetCrypto.PageOverhead;
            int targetHdrSize = targetCrypto.FileHeaderSize;
            uint pageCount = _nextPageId;

            await using var dst = new FileStream(
                destinationPath, FileMode.Create, FileAccess.Write, FileShare.None,
                bufferSize: 1024 * 1024, FileOptions.Asynchronous);

            // 3. Write the new per-file crypto header.
            //    GetFileHeader generates fresh key material (new random salt + key derivation).
            //    For coordinator-based providers this also sets the database salt so that
            //    subsequent sibling providers can be created by the caller.
            if (targetHdrSize > 0)
            {
                var cryptoHeader = new byte[targetHdrSize];
                targetCrypto.GetFileHeader(cryptoHeader);
#if NET6_0_OR_GREATER
                await dst.WriteAsync(cryptoHeader, ct).ConfigureAwait(false);
#else
                await dst.WriteAsync(cryptoHeader, 0, cryptoHeader.Length, ct).ConfigureAwait(false);
#endif
            }

            // 4. Transcrypt all pages.
            //    Source pages are decrypted via ReadPageCore (uses _cryptoProvider / NullCryptoProvider).
            //    Each plaintext page is then re-encrypted with targetCrypto before writing.
            //
            //    ReadPageCore is called without _rwLock here because the engine-level commit
            //    lock (held by StorageEngine.BackupDetailedAsync) prevents any concurrent writes
            //    or file-size changes during backup — the same guarantee that BackupAsync relies on.
            var plainBuf  = System.Buffers.ArrayPool<byte>.Shared.Rent(_config.PageSize);
            var cipherBuf = System.Buffers.ArrayPool<byte>.Shared.Rent(targetPhysPageSize);
            try
            {
                for (uint pageId = 0; pageId < pageCount; pageId++)
                {
                    ct.ThrowIfCancellationRequested();
                    ReadPageCore(pageId, plainBuf.AsSpan(0, _config.PageSize));
                    targetCrypto.Encrypt(pageId, plainBuf.AsSpan(0, _config.PageSize), cipherBuf.AsSpan(0, targetPhysPageSize));
#if NET6_0_OR_GREATER
                    await dst.WriteAsync(cipherBuf.AsMemory(0, targetPhysPageSize), ct).ConfigureAwait(false);
#else
                    await dst.WriteAsync(cipherBuf, 0, targetPhysPageSize, ct).ConfigureAwait(false);
#endif
                }
            }
            finally
            {
                ...
                System.Buffers.ArrayPool<byte>.Shared.Return(cipherBuf, clearArray: false);
            }
